...
DRV_USBHS_DEVICE_ENDPOINT_OBJ gDrvUSBEndpoints [DRV_USBHS_INSTANCES_NUMBER]
                                            [DRV_USBHS_ENDPOINTS_NUMBER * 2];

// *****************************************************************************
/* Function:
      SYS_MODULE_OBJ DRV_USBHS_Initialize( const SYS_MODULE_INDEX index,
...
#ifdef PLIB_USBHS_ExistsClockResetControl
            if(PLIB_USBHS_ExistsClockResetControl(usbID))
            {
                PLIB_USBHS_GlobalInterruptEnable(usbID);
                /* Start the delay here - 3 ms */
                hDriver->timerHandle = SYS_TMR_DelayMS(3);
            }
            else
            {
...
)
{
    ...
    {
        SYS_DEBUG(SYS_ERROR_INFO, "Interrupt Source disable failed");
    }
...
/* Function:
  ...
  Summary:
    Dynamic impementation of DRV_USBHS_Status system interface function.

  Description:
    This is the dynamic impementation of DRV_USBHS_Status system interface
    function.
...
/* Function:
  ...
  Summary:
    Dynamic impementation of DRV_USBHS_Close client interface function.

  Description:
    This is the dynamic impementation of DRV_USBHS_Close client interface
    function.
...
/* Function:
  ...
  Summary:
    Dynamic impementation of DRV_USBHS_Tasks_ISR system interface function.

  Description:
    This is the dynamic impementation of DRV_USBHS_Tasks_ISR system interface
    function.
...
// *****************************************************************************
/* Function:
    void DRV_USBHS_ClientEventCallBackSet(DRV_HANDLE   handle,
                                        uintptr_t    hReferenceData,
                                        DRV_USBHS_EVENT_CALLBACK eventCallBack)

  Summary:
    Dynamic impementation of DRV_USBHS_ClientEventCallBackSet client
    interface function.

  Description:
    This is the dynamic impementation of DRV_USBHS_ClientEventCallBackSet
    client interface function.
