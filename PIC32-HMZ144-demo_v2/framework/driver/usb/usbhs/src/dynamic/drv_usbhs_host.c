...
{
    /* In the host mode, we peform attach debouncing */
    ...
    switch(hDriver->attachState)
    {
        case DRV_USBHS_HOST_ATTACH_STATE_CHECK_FOR_DEVICE_ATTACH:
            ...
            if(hDriver->deviceAttached)
            {
                ...
                {
                    /* The debounce timer has started */
        ...
        case DRV_USBHS_HOST_ATTACH_STATE_DEBOUNCING:
            ...
            /* Check if the debounce delay has completed. If so then
             * check if the device is still attached. */
            if(SYS_TMR_DelayStatusGet(hDriver->timerHandle))
            {
                /* The debounce period has ended. Is the device
        ...
        case DRV_USBHS_HOST_ATTACH_STATE_READY:
            ...
            /* Debouncing is done and device ready. We can check
...
void _DRV_USBHS_HOST_ResetStateMachine(DRV_USBHS_OBJ * hDriver)
{
    ...
    switch(hDriver->resetState)
    {
        case DRV_USBHS_HOST_RESET_STATE_NO_RESET:
            ...
            /* No reset signalling is request */
            break;

        case DRV_USBHS_HOST_RESET_STATE_START:

            /* We should start reset signalling. First try to get a
             * timer */
...
)
{
    ...
    else
    {
        ...
        dmaChannelGrabbed = _DRV_USBHS_HOST_GetFreeDMAChannel((DRV_USBHS_OBJ *)(pipe->hClient),
        ...
        else
        {
            /* DMA CHANNEL has been grabbed and DMA buffer address is OK
               (divisible by 4). So program the DMA registers now to start
                DMA operation
            */
            ((DRV_USBHS_OBJ *)(pipe->hClient))->gDrvUSBDMAPool[dmaChannelGrabbed]
                    .count = count;
...
)
{
    ...
    while(irp != NULL)
    {
        ...
        if(irp->status == USB_HOST_IRP_STATUS_IN_PROGRESS)
        {
             ...
             * the _DRV_USBHS_HOST_NonControlIRPProcess()
             * and _DRV_USBHS_HOST_ControlXferProcess()
             * functions */
            irp->status = USB_HOST_IRP_STATUS_ABORTED;
            if(irp->callback != NULL)
            {
...
void _DRV_USBHS_HOST_Tasks_ISR(DRV_USBHS_OBJ * hDriver)
{
    ...
    else if(interruptStatus & USBHS_GENINT_DEVDISCONN)
    {
         ...
         * while the attach debouncing is in progress. We just set a flag saying
...
void DRV_USBHS_HOST_ROOT_HUB_OperationEnable(DRV_HANDLE handle, bool enable)
{
    ...
    else
    {
        if(true == enable)
        {
            ...
            PLIB_USBHS_InterruptEnableSet(pUSBDrvObj->usbID, (USBHS_GENINT_DEVCONN|USBHS_GENINT_DEVDISCONN), 0x1, 0x0);
