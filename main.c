...
#elif __32MX220F032B__
#pragma config DEBUG = OFF, ICESEL = ICS_PGx3, PWP = OFF, BWP = OFF, CP = OFF, \
               JTAGEN = OFF
#pragma config FNOSC = PRIPLL, FSOSCEN = OFF, IESO = OFF, POSCMOD = XT, \
	       OSCIOFNC = OFF, FPBDIV = DIV_1, FCKSM = CSDCMD, WDTPS = PS1, \
	       FWDTEN = OFF, WINDIS = OFF, FWDTWINSZ = WINSZ_25
#pragma config FPLLIDIV = DIV_1, FPLLMUL = MUL_20, UPLLIDIV = DIV_1, \
	       UPLLEN = 0, FPLLODIV = DIV_2
#pragma config PMDL1WAY = ON, IOL1WAY = ON, FUSBIDIO = ON, FVBUSONIO = ON
#else
...
int main(void)
{
	...
	SYSTEMConfigPerformance(80000000);
        ...
        uart_putstr("sysclock initialized\n\r");
